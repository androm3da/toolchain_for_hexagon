...
[binaries]
c = ['${CC}', '--target=hexagon-unknown-none-elf', '-m${archver}', '-fno-pic', '-fno-PIE', '-static', '-nostdlib', '-fuse-init-array', '-G0']
c_ld = '${LLD}'
ar = '${AR}'
as = '${CC}'
nm = '${NM}'
...
build_clang_hexagon_cross() {
    ...
    for triple in hexagon-unknown-linux-musl hexagon-unknown-none-elf \
                  hexagon-linux-musl hexagon-none-elf hexagon; do
